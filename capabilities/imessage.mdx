...
## Turnkey Remote Agent Execution

Looking to drive local coding assistants (such as **Claude Code**, **Hermes Agent**, **OpenAI Codex**, or **OpenCode**) from your iPhone with zero webhook server setup?

<Card title="Drive Local Agents via iMessage (Agent Connect)" icon="terminal" href="/capabilities/connect">
  Launch a real-time duplex bridge with built-in remote approval escalation for sensitive commands using `wirebox connect`.
</Card>
