...
Wirebox brings autonomous AI agents to **Apple iMessage**. By connecting your agents to Apple's native messaging ecosystem, agents can interact with users through real-time blue bubbles on iPhone, iPad, and Mac — complete with rich media attachments and push notifications.
...
## Gateway User Access Control

Manage allowed user phone numbers on the iMessage gateway to restrict which phone numbers can communicate with agents:
...
with Wirebox() as wb:
    # Add allowed phone number
    wb.imessage.users.add("+15551234567")

    # List registered users
    users = wb.imessage.users.list()
    for u in users.users:
        print(f"User: {u.phone_number} (Added: {u.created_at})")
...
// Add allowed phone number
await wb.imessage.users.add("+15551234567");
