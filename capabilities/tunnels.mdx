...
<Tip>
  **Looking to bridge local coding agents (Claude Code, Hermes, OpenAI Codex, OpenCode) to iMessage or SMS?**

  You do not need to run an HTTP server or proxy ports with `wirebox tunnel connect`. Use **[Agent Connect (`wirebox connect`)](/capabilities/connect)** instead — a turnkey execution harness that establishes an outbound agent session, runs local agent CLI processes on demand, supports remote HITL approval escalations, and can be installed as an OS daemon.
</Tip>
